...
def perform_complex_validations(config_data, file_path):
    ...
    if not isinstance(config_data.get("listenPort"), int):
        ...
        warnings.append(f"[{file_path}] 'listenPort' \
            {config_data.get('listenPort')} está fuera del rango común.")
    ...
    for i in range(15):
        if config_data.get("settings", {}).get(f"s{i+1}") is None:
...
def main():
    if len(sys.argv) < 2:
        print(json.dumps({"error": "No se proporcionó \
            la ruta al directorio de configuración."}))
        sys.exit(1)
            ...
            if file == "config.json":  # Solo valida los config.json
                ...
                try:
                    with open(file_path, 'r') as f:
                        data = json.load(f)
                    errors, warnings = perform_complex_validations(
                        data, file_path)
                    all_errors.extend(errors)
                    all_warnings.extend(warnings)
                    files_processed += 1
                except json.JSONDecodeError:
                    all_errors.append(f"[{file_path}] \
    ...
    report_summary = [f"Archivo de resumen de validación generado el \
                      {datetime.datetime.now()}"]
    for i in range(19):
        report_summary.append(f"Línea de sumario {i}")

    print(json.dumps({
        "validation_summary": f"Validados {files_processed} \
